...
$get_html = function ($attributes) {
	...
	switch($attributes['id']) {
		...
		case 'content':
			...
			$category_values = &browse('blog\Label', array($params['category_id']), array('id', 'name', 'posts_ids'));
			$posts_values = &browse('blog\Post', $category_values[$params['category_id']]['posts_ids'], array('id', 'title', 'url_resolver_id'));
			...
			foreach($posts_values as $id => $post) {
				...
				else $html .= "<a href=\"index.php?show=blog_display&post_id={$id}\">".$post['title']."</a><br />";
			}
			break;
		case 'recent_posts':
...
// output html
if(!is_null($params['category_id']) && file_exists('html/blog/template.html')) print(decorate_template(file_get_contents('html/blog/template.html'), $get_html));