...
$get_html = function ($attributes) {
	...
	switch($attributes['id']) {
		case 'content':
			$post_values = &browse('blog\Post', array($params['post_id']), array('id', 'created', 'title', 'content', 'comments_ids', 'related_posts_ids'));
		...
		case 'related_posts':
			$post_values = &browse('blog\Post', array($params['post_id']), array('id', 'created', 'title', 'content', 'comments_ids', 'related_posts_ids'));
			$related_posts_values = &browse('blog\Post', $post_values[$params['post_id']]['related_posts_ids'], array('id', 'title'));
		...
		case 'recent_posts':
			...
			break;
		case 'comments':
			$comments_values = &browse('blog\Comment', $post_values[$params['post_id']]['comments_ids']);
			$dateFormatter = new DateFormatter();
...
// output html
if(!is_null($params['post_id']) && file_exists('html/blog/template.html')) print(decorate_template(file_get_contents('html/blog/template.html'), $get_html));