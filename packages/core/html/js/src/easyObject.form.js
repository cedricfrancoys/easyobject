...
(function($) {
	...
	*
	*/
// todo: implement a method 'getHeight'	(actual height of the form, as accurate as possible)
...
// todo : would be better to handle this dynamicaly, so we could toggle objects on user actions
			...
			*/
			populate: function($form, conf) {
				...
				//var object_values = (browse(conf.class_name, [conf.object_id], fields, conf.lang))[conf.object_id];
				// we only need values of simple fields
				var object_values = (browse(conf.class_name, [conf.object_id], null, conf.lang))[conf.object_id];
